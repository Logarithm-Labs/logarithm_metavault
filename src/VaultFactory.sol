...
contract VaultFactory is UpgradeableBeacon, IPriorityProvider {
    ...
    event ProxyCreated(
        address indexed proxy, bool indexed upgradeable, address indexed implementation, string name, string symbol
    );
    ...
    error Factory__Implementation();
    error Factory__BadQuery();
    ...
    function createVault(bool upgradeable, string calldata name, string calldata symbol) external returns (address) {
        ...
        if (upgradeable) {
            proxy = address(
                new BeaconProxy(address(this), abi.encodeWithSelector(IMetaVault.initialize.selector, name, symbol))
            );
        } else {
            proxy = _implementation.clone();
            IMetaVault(proxy).initialize(name, symbol);
        }
        ...
        emit ProxyCreated(proxy, upgradeable, _implementation, name, symbol);
    ...
    function getProxyListSlice(uint256 start, uint256 end) external view returns (address[] memory list) {
        ...
        if (end < start || end > proxyList.length) revert Factory__BadQuery();
